...
import (
	...
	sap_api_output_formatter "sap-api-integrations-debit-memo-request-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callDebitMemoRequestSrvAPIRequirementHeader("A_DebitMemoRequest", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callDebitMemoRequestSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	url := fmt.Sprintf("A_DebitMemoRequestItem", item)
	outputDataItem, err := c.callDebitMemoRequestSrvAPIRequirementItem(url, item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callDebitMemoRequestSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
